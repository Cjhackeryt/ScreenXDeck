...
This repository is the implementation for **ScreenXDeck** (`com.cjhackeryt.screenxdeck`), an out-of-process Windows display modes, monitor control, window management, virtual desktops, and brightness plugin for **Macro Deck 3**.

[README.md](README.md) is the human-facing guide: how to build, how to run against a real host, and how to pack. This file is the rule set for developing and maintaining the plugin. Read it before changing code.

## Orientation

```
ScreenXDeck.slnx                 solution file referencing plugin and test projects
ScreenXDeckPlugin/
  ScreenXDeckPlugin.csproj       the plugin project targeting net10.0-windows (win-x64)
  Program.cs                     builder chain: CreatePlugin -> Services -> UseMacroDeckLogging -> UseLocalization -> RegisterIntegration -> Build -> RunAsync
  ...
  ScreenXDeckIntegration.cs      the integration: actions, variables, surfaces, and widget wiring
  DisplayModeService.cs          Windows display topology (internal, clone, extend, external via SetDisplayConfig/DisplaySwitch)
  MonitorBrightnessService.cs    hybrid monitor brightness engine: WMI (internal) + DDC/CI (external) + GPU gamma ramp fallback
  RefreshRateService.cs          monitor refresh rate querying and switching (EnumDisplaySettings/ChangeDisplaySettingsEx)
  DisplayVariables.cs            variable definitions and identifiers for monitor states and brightness
  Actions/                       action definitions for display modes, brightness, refresh rates, windows, and desktops
  Monitors/                      monitor discovery, DDC/CI communication, and gamma ramp handling
  Widgets/                       MacroTools monitor-brightness widget
  Windows/                       window focus, minimize, maximize, snap, and virtual desktop services
  Localization/
    Strings.resx                 default-culture strings; Strings.<tag>.resx per language
  Assets/
    icon.svg                     the icon the manifest declares
tests/ScreenXDeck.Tests/
  ScreenXDeck.Tests.csproj       unit test project
  IntegrationTests.cs            tests for actions, variables, parameter parsing, and catalog wiring
```
...
diagnostics that catch most mistakes while you type, plus the `[MacroDeckSdkUsage]`
...
```bash
macrodeck-plugin test --artifact ./artifacts/com.cjhackeryt.screenxdeck-0.1.0.macroDeckPlugin --report markdown --output conformance.md
```
...
```bash
macrodeck-plugin build --source ScreenXDeckPlugin --output ./artifacts
macrodeck-plugin inspect --artifact ./artifacts/com.cjhackeryt.screenxdeck-0.1.0.macroDeckPlugin
```
